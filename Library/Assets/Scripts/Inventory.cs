...
public class Inventory : MonoBehaviour
{
    ...
    public Inventory(){
    	...
    	fins = true;*/
    	itemList.Add(new ResourceItem(ResouceItemType.Fishcan, 4));
    	itemList.Add(new ResourceItem(ResouceItemType.Seaweed, 0));
    	itemList.Add(new ResourceItem(ResouceItemType.Wood, 20));
...
/*Seaweed,	Fishcan,	Wood,	Metal, 	Bolt,	Spear,	Knife, 	Trident,	Harpoon,	RadSuit,	HeatSuit,	Fins*/
	...
	public int ReturnItemCount(ResouceItemType item)
	{
		foreach (ResourceItem i in itemList)
		{
			if (i.itemType == item)
	...
	public void SetItemCount(ResouceItemType item, int amount)
    {
		foreach(ResourceItem i in itemList)
        {
			if(i.itemType == item)
